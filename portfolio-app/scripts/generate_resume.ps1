...
$tempHtmlPath = Join-Path $scriptDir "resume_temp.html"
$outputPdfDir = Join-Path $scriptDir "..\public"
$outputPdfPath = Join-Path $outputPdfDir "Vishnu_Priya_Resume.pdf"

# Write HTML content to temp file
Write-Host "Writing temporary HTML..."
$htmlContent | Out-File -FilePath $tempHtmlPath -Encoding utf8

# Ensure output directory exists
if (-not (Test-Path $outputPdfDir)) {
    New-Item -ItemType Directory -Path $outputPdfDir -Force | Out-Null
}
...
Write-Host "Compiling PDF using Google Chrome..."
...
if (Test-Path $outputPdfPath) {
    $size = (Get-Item $outputPdfPath).Length
    Write-Host "SUCCESS: PDF generated at $outputPdfPath ($size bytes)"
} else {
    Write-Error "FAILED: PDF file was not generated."
}
